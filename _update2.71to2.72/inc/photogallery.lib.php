...
class kPhotogallery {
	...
	public function getMetadata($dir="false",$ll=false) {
		...
		if(isset($dir[1])&&$dir[1]!="") {
			$query="SELECT idphg,titolo,traduzioni,template FROM ".TABLE_PHOTOGALLERY." WHERE (`dir`='".b3_htmlize($GLOBALS['__subsubdir__'],true,"")."' OR `dir`='".ksql_real_escape_string($GLOBALS['__subsubdir__'])."') AND ll='".$ll."' LIMIT 1";
				$results=ksql_query($query);
