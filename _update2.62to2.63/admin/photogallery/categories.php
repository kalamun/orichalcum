...
if(isset($_POST['save'])&&count($_POST['cat'])>0) {
	...
	foreach($order as $ref=>$categories) {
		foreach($categories as $ordine=>$idcat) {
			if(!$kaCategorie->updateOrder($idcat,$ordine+1,$ref,TABLE_PHOTOGALLERY)) $log="Errore durante il salvataggio dell'ordinamento";
			}
		}
...
elseif(isset($_POST['sortAZ'])) {
	$log=$kaCategorie->sortby('`categoria` ASC',TABLE_PHOTOGALLERY);
	}

elseif(isset($_POST['insert'])) {
	$log=$kaCategorie->add($_POST['titolo'],$_POST['dir'],TABLE_PHOTOGALLERY);
	if($log==false) {
...
elseif(isset($_GET['delete'])) {
	...
	$log=$kaCategorie->del($_GET['delete'],TABLE_PHOTOGALLERY);
	if($log==false) {
...
if(isset($_GET['idcat'])) {
	//modifica
	$cat=$kaCategorie->get($_GET['idcat'],TABLE_PHOTOGALLERY);
	?>
