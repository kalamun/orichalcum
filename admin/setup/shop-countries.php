...
if(isset($_POST['update'])) {
	...
	?><div id="MsgSuccess">Zone salvate con successo</div><?
...
<div class="subset">
	...
	<div class="inactive">
		...
		foreach($countriesUnused as $code=>$country) {
			?><li countryCode="<?= $code; ?>"><?= $country; ?></li><?
			}
			?></ul>
		</div>
	</div>
	...
	for($i=1;$i<=7;$i++) { ?>
		<div>
			...
			<div class="zoneContainer" id="zoneContainer<?= $i; ?>">
				...
				$results=mysql_query($query);
				while($row=mysql_fetch_array($results)) {
					?><li countryCode="<?= $row['ll']; ?>"><img src="<?= BASEDIR; ?>img/lang/<?= strtolower($row['ll']); ?>.gif" /> <?= $row['country']; ?></li><?
					}
