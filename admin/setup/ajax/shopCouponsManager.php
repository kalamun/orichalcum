...
<div style="padding:30px;">
	<table class="tabella hidebuttons">
		...
		foreach($kaShop->getCouponCodesList(array('idscoup'=>$_GET['idscoup'],'valid'=>$_GET['valid'])) as $m) { ?>
			...
			<td><?= $m['code']; ?></td>
			<td>
				<? if($_GET['valid']==1) { ?><a href="?valid=<?= $_GET['valid']; ?>&idscoup=<?= $_GET['idscoup']; ?>&markasused=<?= $m['code']; ?>" class="smallbutton">Segna come utilizzato</a><? }
					elseif($_GET['valid']==0) { ?><a href="?valid=<?= $_GET['valid']; ?>&idscoup=<?= $_GET['idscoup']; ?>&markasvalid=<?= $m['code']; ?>" class="smallbutton">Segna come valido</a><? } ?>
