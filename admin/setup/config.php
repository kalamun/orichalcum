...
<form action="?" method="post">
	<textarea style="width:100%;height:400px;" name="contents"><?
		$c=$kaConfigInc->read();
	...
	if(file_exists($kaConfigInc->getFilename().'.bkup.php')) {
		?><input type="submit" name="recover" value="Ripristina la copia di back-up del <?= date("d-m-Y (H:i)",filemtime($kaConfigInc->getFilename().'.bkup.php')); ?>" class="button" onclick="return confirm('Sei sicuro di voler ripristinare la copia di back-up?');" /><?
		}
	?>
	</div>
	</form></div><br /><br />
