...
?>
<h1><?php  echo $kaTranslate->translate(PAGE_NAME); ?></h1>
<br />
...
if(isset($_GET['check']))
{
	...
	foreach($kaMembers->getUsersList(array("groupby"=>"email", "conditions"=>"`status`='act'")) as $m)
	{
		...
		if(substr_count($domain,".")>1) $domain=preg_replace("/^.*?\.([^\.]{4,128})((\.[A-Z]{2,3})+)$/i","$1$2",$domain);
		if($error=="")
		{
		...
		<tr id="entry<?= $m['idmember']; ?>">
			...
			<td id="error<?= $m['idmember']; ?>"><?= $kaTranslate->translate($error); ?></td>
