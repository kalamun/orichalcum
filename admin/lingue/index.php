...
?><h1><? echo PAGE_NAME; ?></h1>
	...
	<div class="DragZone">
	<ul class="dragdrop lingue"><?
		$query="SELECT * FROM ".TABLE_LINGUE." ORDER BY ordine";
		$results=mysql_query($query);
		$i=0;
		while($row=mysql_fetch_array($results)) {
			?><li><? echo '<strong>'.$row['lingua'].'</strong><div class="small">'.$row['ll'].' - '.($row['online']=='s'?'ONLINE':'OFFLINE').'</div>'; ?><input type="hidden" name="lingua[]" value="<? echo $row['idli']; ?>" /><span><a href="javascript:kOpenIPopUp('modifica.php','idli=<? echo $row['idli']; ?>','600px','400px')"><img src="<? echo ADMINRELDIR; ?>/img/12edit.gif" width="12" height="12" alt="modifica" /></a><a href="?delete=<? echo $row['idli']; ?>" onclick="return confirm('Sei sicuro di voler eliminare questa voce?');"><img src="<? echo ADMINRELDIR; ?>/img/12close.gif" width="12" height="12" alt="elimina" /></a></span></li><? $i++; }
		?></ul>
		</div>
	<br />
