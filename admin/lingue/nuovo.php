...
<div style="padding:20px;">
	<div class="box">Scegli una lingua <select onchange="kSelectNewLang();" id="langTemplates"><option value="">Personalizzata...</option><?
		$ll=file('languages.txt');
		foreach($ll as $l) {
			$l=explode("\t",trim($l));
			?><option value="<?= trim($l[1]).trim($l[2]).trim($l[0]); ?>"><?= $l[0]; ?></option><?
			}
		?></select>
		</div>
	<br /><br />
