...
class kaStats {
	...
	public function createNewslettersSummary()
	{
		...
		foreach($kaNewsletter->getArchiveList( array("stats"=>false, "conditions"=>"`data`<'".strftime("%Y-%m-%d %H:%M:%S",time() - 1296000)."'") ) as $list)
		{
			...
			if($row==false)
			{
				$sent=$kaNewsletter->getLogCount(array("idarch"=>$list['idarch'], "read"=>false));
				$read=$kaNewsletter->getLogCount(array("idarch"=>$list['idarch'], "read"=>true));
				$readfrom=implode("\n",$kaNewsletter->getReadFrom(array("idarch"=>$list['idarch'], "read"=>true)));
				$this->insertSummaryEntry("newsletter", "recipients", $list['idarch'], $sent, '');
				$this->insertSummaryEntry("newsletter", "read", $list['idarch'], $read, $readfrom);
