...
if(isset($_GET['idnews'])) {
	...
	<?= (strpos($pageLayout,",date,")!==false?$kaTranslate->translate('News:Created').' '.strftime("%d %b %Y",mktime(1,0,0,substr($row['data'],5,2),substr($row['pubblica'],8,2),substr($row['data'],0,4))).' '.substr($row['data'],11,5).'<br />':''); ?>
	...
	<?= (strpos($pageLayout,",expiration,")!==false?$kaTranslate->translate('News:Expiration').' '.strftime("%d %b %Y",mktime(1,0,0,substr($row['scadenza'],5,2),substr($row['pubblica'],8,2),substr($row['scadenza'],0,4))).' '.substr($row['scadenza'],11,5).'<br />':''); ?>
	<? }
