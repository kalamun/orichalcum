	...
	?><form action="?idnews=<?= $row['idnews']; ?>" method="post" onsubmit="return checkForm();">
		...
		if($kaImpostazioni->getVar('news-commenti',1)=='s'||$row['commentiTot']>0) { ?>
			<fieldset class="box"><legend><?= $kaTranslate->translate('News:Comments'); ?></legend>
				...
				<a href="javascript:k_openIframeWindow(ADMINDIR+'inc/commentsManager.php?t=<?= TABLE_NEWS; ?>&id=<?= $row['idnews']; ?>','600px','400px')" class="smallbutton"><?= $kaTranslate->translate('News:Comments management'); ?></a>
				</fieldset><br />
			<?php  } ?>
		...
		if($kaImpostazioni->getVar('facebook',1)=='s') { ?>
			<fieldset class="box"><legend>Facebook</legend>
				<div class="newCat"><a href="javascript:k_openIframeWindow('ajax/facebook.php?id=<?= $row['idnews']; ?>','600px','80%')" class="smallbutton"><?= $kaTranslate->translate('News:Create a facebook post'); ?></a></div>
			</fieldset><br />
			<?php  } ?>
