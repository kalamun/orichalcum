	...
	if(count($variations)==0) echo $kaTranslate->translate('Shop:There are no variations for this item');
		...
		foreach($variations as $row) {
			if($bkupcollection!=$row['collection']) {
				if($bkupcollection!="") { ?></tbody></table><div style="clear:both;"></div><br /><? }
				?><div style="float:left;margin:10px 20px 10px 0;"><h3><?= $row['collection']; ?></h3></div><table class="tabella" style="float:left;margin:10px 20px 10px 0;"><tbody class="DragZone"><?
				} ?>
			<tr>
