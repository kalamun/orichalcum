...
if(!isset($_GET['idpag'])) {
	...
	if(isset($_GET['addtomenu'])) {
		...
		$query="SELECT * FROM ".TABLE_PAGINE." WHERE idpag='".$_GET['usePage']."' AND ll='".$_SESSION['ll']."' LIMIT 1";
		...
		if($page=mysql_fetch_array($results)) {
			...
			if($log!="") {
				...
				$kaLog->add("ERR",'Pages: Error while inserting in the menu the page <a href="'.BASEDIR.strtolower($_SESSION['ll']).'/'.$dir.'">'.$titolo.'</a> <em>(ID: '.$id.')</em>');
				}
			else {
				echo '<div id="MsgSuccess">Pagina inserita nel men&ugrave;</div>';
				$kaLog->add("INS",'Pages: Page was inserted in the menu: <a href="'.BASEDIR.strtolower($_SESSION['ll']).'/'.$dir.'">'.$titolo.'</a> <em>(ID: '.$id.')</em>');
				}
			}		
		}
		...
		while($page=mysql_fetch_array($results)) { ?>
			...
			<td>
				<h2><a href="?idpag=<?= $page['idpag']; ?>">
					<?= $page['titolo']; ?>
					<? if($page['riservata']=='s') echo '<small class="alert">'.$kaTranslate->translate('Pages:DRAFT').'</small>'; ?>
					<? if($page['allowconversions']=='1') echo '<small class="alert conversions">'.$kaTranslate->translate('Pages:CONVERSIONS').'</small>'; ?>
					</a></h2>
				<small class="actions">
	...
	<div class="subset">
		...
		<? if(strpos($pageLayout,",categories,")!==false) { ?>
			<div class="box closed"><h2 onclick="kBoxSwapOpening(this.parentNode);"><?= $kaTranslate->translate('Pages:Categories'); ?></h2>
				...
				<script type="text/javascript">k_reloadCat(<? echo $page['idpag']; ?>);</script>
				</div>
			<br />
			<? } ?>
		...
		<? if(strpos($pageLayout,",seo,")!==false) {
			...
			<table>
				<tr>
					...
					<td><input type="text" name="seo_title" id="seo_title" style="width:300px;" value="<? $md=$kaMetadata->get(TABLE_PAGINE,$page['idpag'],'seo_title'); echo b3_lmthize($md['value'],"input"); ?>" /></td>
					</tr>
				<tr>
					<td><label for="seo_priority"><?= $kaTranslate->translate('Pages:Priority'); ?></label></td>
					<td><input type="text" name="seo_priority" id="seo_priority" style="width:50px;" value="<? $md=$kaMetadata->get(TABLE_PAGINE,$page['idpag'],'seo_priority'); echo b3_lmthize($md['value'],"input"); ?>" /></td>
					<td><label for="seo_description"><?= $kaTranslate->translate('Pages:Description'); ?></label></td>
					<td><input type="text" name="seo_description" id="seo_description" style="width:300px;" value="<? $md=$kaMetadata->get(TABLE_PAGINE,$page['idpag'],'seo_description'); echo b3_lmthize($md['value'],"input"); ?>" /></td>
					</tr>
				<tr>
					<td colspan="2">
					<td><label for="seo_keywords"><?= $kaTranslate->translate('Pages:Keywords'); ?></label></td>
					<td><input type="text" name="seo_keywords" id="seo_keywords" style="width:300px;" value="<? $md=$kaMetadata->get(TABLE_PAGINE,$page['idpag'],'seo_keywords'); echo b3_lmthize($md['value'],"input"); ?>" /></td>
					</tr>
				<tr>
					<td colspan="2">
						<input type="checkbox" name="seo_robots[]" id="seo_robots_noindex" value="noindex" <? $md=$kaMetadata->get(TABLE_PAGINE,$page['idpag'],'seo_robots'); if(strpos($md['value'],"noindex")!==false) { echo 'checked'; }; ?> /> <label for="seo_robots_noindex"><?= $kaTranslate->translate('Pages:No index'); ?></label>,
						<input type="checkbox" name="seo_robots[]" id="seo_robots_nofollow" value="nofollow" <? $md=$kaMetadata->get(TABLE_PAGINE,$page['idpag'],'seo_robots'); if(strpos($md['value'],"nofollow")!==false) { echo 'checked'; }; ?>/> <label for="seo_robots_nofollow"><?= $kaTranslate->translate('Pages:No follow'); ?></label>,
						<input type="checkbox" name="seo_robots[]" id="seo_robots_noarchive" value="noarchive" <? $md=$kaMetadata->get(TABLE_PAGINE,$page['idpag'],'seo_robots'); if(strpos($md['value'],"noarchive")!==false) { echo 'checked'; }; ?>/> <label for="seo_robots_noarchive"><?= $kaTranslate->translate('Pages:No archive'); ?></label>
						</td>
					<td><label for="seo_canonical">Canonical URL</label></td>
					<td><input type="text" name="seo_canonical" id="seo_canonical" style="width:300px;" value="<? $md=$kaMetadata->get(TABLE_PAGINE,$page['idpag'],'seo_canonical'); echo b3_lmthize($md['value'],"input"); ?>" /></td>
					</tr>
				</table>
			</div><?
			} ?>
