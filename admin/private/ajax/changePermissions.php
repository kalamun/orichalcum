	...
	<div style="padding:20px;">
		...
		<div style="width:48%;float:left;">
			...
			<select name="permissions" onchange="swapMembersListVisualization(this,'membersList');">
				...
				<? if(isset($allowedpermissions['public'])) { ?><option value="public"<?= ($permissions['permissions']=='public'&&$permissions['inherited']!=true?' selected':''); ?>><?= $kaTranslate->translate('Private:Anyone can access'); ?></option><? } ?>
				<? if(isset($allowedpermissions['members'])) { ?><option value="members"<?= ($permissions['permissions']=='members'&&$permissions['inherited']!=true?' selected':''); ?>><?= $kaTranslate->translate('Private:All members can access'); ?></option><? } ?>
				<? if(isset($allowedpermissions['restricted'])) { ?><option value="restricted"<?= ($permissions['permissions']=='restricted'&&$permissions['inherited']!=true?' selected':''); ?>><?= $kaTranslate->translate('Private:Only specified members can access'); ?></option><? } ?>
				<? if(isset($allowedpermissions['private'])) { ?><option value="private"<?= ($permissions['permissions']=='private'&&$permissions['inherited']!=true?' selected':''); ?>><?= $kaTranslate->translate('Private:Nobody can access'); ?></option><? } ?>
				</select><br />
			<br />
