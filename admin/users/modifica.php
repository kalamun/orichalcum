...
if(isset($_POST['update'])&&isset($_POST['n_username'])&&$_POST['n_username']!=""&&isset($_POST['n_name'])&&isset($_GET['iduser'])) {
	$log="";
	if(!$kaUsers->update($_GET['iduser'],$_POST['n_name'],$_POST['n_email'],$_POST['n_username'],$_POST['n_p'])) $log="Problemi durante la modifica dell'utente";
...
if(isset($_GET['iduser'])) {
	...
	?>
	<div class="subset">
		<strong><?= $kaTranslate->translate('Users:Profile picture'); ?></strong>
		<iframe src="<?php echo ADMINDIR; ?>inc/imgallery.inc.php?refid=imgallery&mediatable=<?php echo TABLE_USERS; ?>&mediaid=<?php echo $u['iduser']; ?>&start=1&max=1&label=<?= urlencode($kaTranslate->translate('Users:Upload a photo of you')); ?>" class="imgframe" id="imgallery"></iframe>
		<br />
		</div>
