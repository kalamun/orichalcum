...
class kaUsers {
	...
	public function add($name,$email,$username,$password,$permissions=false) {
		...
		if($row=mysql_fetch_array($results)) return false;
			...
			$query="INSERT INTO ".TABLE_USERS." (`name`,`email`,`username`,`password`,`permissions`,`created`,`lastlogin`) VALUES('".b3_htmlize($name,true,"")."','".b3_htmlize($email,true,"")."','".b3_htmlize($username,true,"")."','".md5($password)."','".$perm."',NOW(),NOW())";
			return mysql_query($query)?true:false;
			}
		}

	/* update user */
	public function update($iduser,$name,$email,$username,$permissions=false) {
		...
		$query.="WHERE `iduser`=".mysql_real_escape_string($iduser)." LIMIT 1";
		return mysql_query($query)?true:false;
		}
	...
	public function propGetList($iduser) {
		$output=array();
		$query="SELECT * FROM ".TABLE_USERS_PROP." WHERE iduser='".$iduser."'";
		$results=mysql_query($query);
		while($row=mysql_fetch_array($results)) {
