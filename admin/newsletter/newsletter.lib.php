...
class kaNewsletter {
	...
	public function getNewslettersList($vars=array()) {
		...
		while($row=mysql_fetch_array($results)) {
			$row['subscribers_number']=count($this->kaMembers->getUsersList(array("lists"=>array($row['idlista']))));
			$output[]=$row;
			}
		return $output;
		}
	...
	public function getArchiveList($vars) {
		...
		$output=array();
		$query="SELECT * FROM ".TABLE_NEWSLETTER_ARCH." ORDER BY `data` DESC LIMIT ".intval($vars['from']).",".intval($vars['limit']);
		$results=mysql_query($query);
		while($row=mysql_fetch_array($results)) {
