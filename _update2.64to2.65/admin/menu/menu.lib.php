...
class kaMenu {
	...
	public function addElement($vars)
	{
		...
		*/
		...
		if($vars['where']=="after")
		{
			$query="UPDATE `".TABLE_MENU."` SET `ordine`=`ordine`+1 WHERE `ref`='".intval($ref)."' AND `ordine`>='".intval($order)."' AND `idmenu`<>'".$idmenu."' AND `collection`='".mysql_real_escape_string($collection)."' AND `ll`='".mysql_real_escape_string($_SESSION['ll'])."'";
			mysql_query($query);
