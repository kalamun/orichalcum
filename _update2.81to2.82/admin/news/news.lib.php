...
class kaNews {
	...
	public function add($values,$sottotitolo="",$anteprima="",$testo="",$categorie="",$data="",$pubblica="",$scadenza="",$template="",$layout="",$traduzioni="",$dir="",$home="s",$calendario="s",$iduser=false,$ll=false) {
		...
		if(isset($values['copyfrom'])&&is_numeric($values['copyfrom'])) {
			...
			if($row=ksql_fetch_array($results)) {
				$query="INSERT INTO `".TABLE_NEWS."` (`online`,`titolo`,`sottotitolo`,`anteprima`,`testo`,`featuredimage`,`photogallery`,`data`,`pubblica`,`starting_date`,`scadenza`,`modified`,`template`,`layout`,`traduzioni`,`categorie`,`dir`,`home`,`calendario`,`iduser`,`ll`)
					SELECT `online`,`titolo`,`sottotitolo`,`anteprima`,`testo`,`featuredimage`,`photogallery`,`data`,`pubblica`,`starting_date`,`scadenza`,`modified`,`template`,`layout`,`traduzioni`,`categorie`,`dir`,`home`,`calendario`,`iduser`,`ll` FROM `".TABLE_NEWS."` WHERE `idnews`='".$values['copyfrom']."' LIMIT 1
					";
	...
	public function update($vars) {
		...
		if(isset($vars['online'])) $vars['online']= ($vars['online']=='y' ? 'y' : 'n');
