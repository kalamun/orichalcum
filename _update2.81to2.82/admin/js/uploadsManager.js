...
function kUploads()
{
	...
	var form=null,scrollContainer=null,fileList=null,formData=null,queue=Array(),currentFile=0,uploadFileProgress=null,search=null,orderby=null,onsubmit=null,onsubmit2=null,limit=1000,lastSelectedItem=null,internetUploadDialog=null,editDialog=null,shiftPressed=false,fileType="",timer=null,scrolling=false;
	
	var init=function(formObject,dropObject,browseObject,editorObject,internetUploadObject,scrollListContainer,fileListContainer,searchContainer,orderbyContainer,onsubmitContainer,onsubmitContainer2,onsubmitAction,onsubmitAction2,limitNumber) {
	...
	var loadImages=function(offset,itemsperpage,conditions)
	{
		if(isLoading==false) // prevend double calls
		{
			...
			aj.send('post','../inc/ajax/uploadHandler.php','&action=getImageList&fileType='+encodeURIComponent(fileType)+'&offset='+parseInt(offset)+'&limit='+parseInt(itemsperpage)+'&orderby='+orderKey+'&search='+encodeURIComponent(searchKey)+'&conditions='+encodeURIComponent(conditions));
	...
	var reloadImage=function(id,select)
	{
		...
		aj.onFail( function() { alert('loading error :-('); } );
		aj.send('post','../inc/ajax/uploadHandler.php','&action=getImageList&fileType='+encodeURIComponent(fileType)+'&offset='+parseInt(offset)+'&limit='+parseInt(itemsperpage)+'&orderby='+orderKey+'&search=&conditions='+encodeURIComponent(conditions));
	}
	...
	var selectImage=function(e,img)
	{
		...
		if(lastSelectedItem && shiftPressed) // multiple selection with Shift
		{
			...
			{
				if(fileList.childNodes[i]==img) offset=i;
				else if(fileList.childNodes[i]==lastSelectedItem) end=i;
			}

			if(end<offset) end = [offset, offset = end][0]; // swap values
			if(status=='selected' && end-offset+fileList.querySelectorAll('.selected').length>=limit) end=offset+limit-fileList.querySelectorAll('.selected').length;
			if(offset==-1 || end==-1 || end==offset) return false;
	...
	var onScrollHandler=function()
	{
		...
		var clientHeight = scrollContainer.offsetHeight;
		var pageHeight = scrollContainer.scrollHeight;
		console.log(scrollTop + ' - ' + clientHeight + ' - ' + pageHeight + ' = ' + (pageHeight-clientHeight-scrollTop) );
	...
	var onDrop=function(e)
	{
		...
		if(isLoading==false) offsetUploading();
	}
	...
	var uploadFile=function(file)
	{
			...
			if(xhr.upload && file.size<=maxFileSize)
			{
				...
				xhr.onreadystatechange=function(e)
				{
					if(xhr.readyState===4)
					{
						...
						isLoading=false;
						offsetUploading(); // upload next file
					}
				};
		...
		// internet upload
			...
			aj.send('post','../inc/ajax/uploadHandler.php','&action=offsetInternetUpload&url='+encodeURIComponent(file.url)+'&copy='+encodeURIComponent(file.copy));
