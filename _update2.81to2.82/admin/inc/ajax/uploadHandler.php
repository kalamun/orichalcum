...
if(isset($_POST['action']))
{
	...
	if($_POST['action']=='getImageList')
	{
		...
		{
			$tmp=array("offset"=>$_POST['offset'], "limit"=>$_POST['limit']);
			$_POST['offset']=0;
			$_POST['limit']=false;
		}
		
		$vars=array();
		$vars['offset'] = $_POST['offset'];
		...
		if(strpos($_POST['orderby'],'filename')!==false)
		{
			...
			{
				if($i < $tmp['offset']) { $i++; continue; }
				$output[$idimg]=$filename;
				if($i >= $tmp['offset']+$tmp['limit']) break;
				$i++;
			}
		}
	...
	elseif($_POST['action']=='offsetInternetUpload')
	{
