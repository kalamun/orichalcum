...
class kaLog
{
	private $kaImpostazioni,$expiration;
	...
	function __construct()
	{
		...
		if(!empty($this->expiration)) $this->clear($this->expiration);
	}
	...
	{
		$azioni=array("INS"=>"Write","UPD"=>"Update","DEL"=>"Delete","CFG"=>"Config","ERR"=>"Error","GEN"=>"Generic");
		$output=array();
		...
		for($i=0;$row=ksql_fetch_array($results);$i++)
		{
			...
			$output[$i]['azione']=$azioni[$row['type']];
		}
		return $output;
	}
