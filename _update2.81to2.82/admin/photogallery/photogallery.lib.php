...
class kaPhotogallery {
	...
	function add($vars)
	{
		...
		$query="SELECT `idphg` FROM `".TABLE_PHOTOGALLERY."` WHERE `ll`='".$vars['ll']."' AND `dir`='".ksql_real_escape_string($vars['dir'])."' LIMIT 1";
		...
		$query="INSERT INTO `".TABLE_PHOTOGALLERY."` (
				...
				'',
				'',
				'".ksql_real_escape_string($vars['translations'])."',
				'".intval($order)."',
				'".ksql_real_escape_string($vars['ll'])."',
				NOW(),
				NOW()
				)";
	...
	function getById($idphg) {
		$query="SELECT * FROM ".TABLE_PHOTOGALLERY." WHERE idphg='".ksql_real_escape_string($idphg)."' LIMIT 1";
		$results=ksql_query($query);
		$row=ksql_fetch_array($results);
	...
	function getTitleById($idphg) {
		$query="SELECT `idphg`,`titolo`,`dir` FROM `".TABLE_PHOTOGALLERY."` WHERE `idphg`='".ksql_real_escape_string($idphg)."' LIMIT 1";
		$results=ksql_query($query);
		$row=ksql_fetch_array($results);
		return $row;
		}
	...
	function update($idphg,$vars) {
		...
		if(isset($vars['dir']))
		{
			$query="SELECT `idphg` FROM `".TABLE_PHOTOGALLERY."` WHERE `ll`='".$vars['ll']."' AND `dir`='".ksql_real_escape_string($vars['dir'])."' AND `idphg`<>'".intval($idphg)."' LIMIT 1";
			$results=ksql_query($query);
			$row=ksql_fetch_array($results);
			if($row!=false) $vars['dir']=rand(10000,99999).$vars['dir'];
		}
		...
		if(isset($vars['text'])) $query.="`testo`='".ksql_real_escape_string($vars['text'])."', ";
		...
		if(isset($vars['categories'])) $query.="`categories`='".ksql_real_escape_string($vars['categories'])."', ";
		$query.="`ll`='".ksql_real_escape_string($vars['ll'])."', `modified`=NOW() WHERE `idphg`=".intval($idphg)." LIMIT 1";
		if(!ksql_query($query)) return false;
		else return $idphg;
		}
	...
	public function setTranslations($idphg,$translations) {
		$query="UPDATE ".TABLE_PHOTOGALLERY." SET `traduzioni`='".ksql_real_escape_string($translations)."' WHERE `idphg`='".ksql_real_escape_string($idphg)."' LIMIT 1";
		if(ksql_query($query)) return true;
		else return false;
		}
	public function removePageFromTranslations($idphg) {
		$query="UPDATE ".TABLE_PHOTOGALLERY." SET `traduzioni`=REPLACE(`traduzioni`,'=".ksql_real_escape_string($idphg)."|','=|') WHERE `traduzioni` LIKE '%=".ksql_real_escape_string($idphg)."%|'";
		if(ksql_query($query)) return true;
		else return false;
		}
