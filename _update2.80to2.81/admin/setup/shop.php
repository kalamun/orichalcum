	...
	<table class="tabella">
		<thead>
			...
			{ ?>
				<tr>
					<td><?= $field['name']; ?><br />
						<small class="actions"><a href="javascript:kOpenIPopUp('ajax/shopEditCustomField.php','idsfield=<?= $field['idsfield']; ?>','800px','400px');"><?= $kaTranslate->translate('UI:Edit'); ?></a> | <a href="?deleteCustomField=<?= $field['idsfield']; ?>" class="delete" onclick="return confirm('<?= addslashes($kaTranslate->translate('Setup:Do you really REALLY want to permanently delete this field?')); ?>');"><?= $kaTranslate->translate("UI:Delete"); ?></a></small>
						</td>
					<td><?= $field['type']; ?></td>
		...
		<br />
		<input type="button" value="<?= $kaTranslate->translate('Setup:Add a field'); ?>" class="button" onclick="kOpenIPopUp('ajax/shopNewCustomField.php','','800px','400px')" />
	</td></tr></table>
