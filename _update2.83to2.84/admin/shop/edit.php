...
if(isset($_POST['update'])&&isset($_GET['idsitem']))
{
	...
	$vars['scontato'] = isset($_POST['scontato']) ? number_format($_POST['scontato'],2,'.','') : 0;
...
if($_GET['search']=="" && empty($_GET['filter']))
{
	...
	if(!empty($_GET['filter']['categories']))
	{
		...
		$vars['conditions'].="(";
		foreach($_GET['filter']['categories'] as $idcat) {
			$vars['conditions'].="`categorie` LIKE '%,".intval($idcat).",%' AND ";
		}
		$vars['conditions'].="`categorie`<>'')";
	}
	
	if(!empty($_GET['filter']['price']['min']))
	{
		if($vars['conditions']!="") $vars['conditions'].=" AND ";
		$vars['conditions'].=" `prezzo`>='".floatval($_GET['filter']['price']['min'])."' ";
	}
	
	if(!empty($_GET['filter']['price']['max']))
	{
		if($vars['conditions']!="") $vars['conditions'].=" AND ";
		$vars['conditions'].=" `prezzo`<='".floatval($_GET['filter']['price']['max'])."' ";
	}
	
	if(!empty($_GET['filter']['status']))
	{
		if($vars['conditions']!="") $vars['conditions'].=" AND ";
		if($_GET['filter']['status']=="online") $vars['conditions'].=" `online`='y' ";
		elseif($_GET['filter']['status']=="draft") $vars['conditions'].=" `online`='n' ";
	}
	
	if(!empty($_GET['filter']['manufacturer']))
	{
		if($vars['conditions']!="") $vars['conditions'].=" AND ";
		$vars['conditions'].=" `manufacturer`='".intval($_GET['filter']['manufacturer'])."' ";
 	}
...
if(!isset($_GET['idsitem'])) { ?>
	...
	<?php 
	/* if there are some filters, show them */
	if($_GET['search']!="" || !empty($_GET['filter'])) { ?>
		<div class="box pager" style="text-align:center;">
			<?
			echo $kaTranslate->translate('Shop:You are filtering results by');
			if(!empty($_GET['search'])) echo ' '.$kaTranslate->translate('Shop:keyword').' "<strong>'.$_GET['search'].'</strong>"';
			if(!empty($_GET['filter']['categories'])) {
				echo ' '.$kaTranslate->translate('Shop:categories');
				$i=0;
				foreach($kaCategorie->getList(TABLE_SHOP_ITEMS) as $cat)
				{
					if(isset($_GET['filter']['categories'][$cat['idcat']]))
					{
						echo ' <strong>'.$cat['categoria'].'</strong> ';
						$i++;
						if($i<count($_GET['filter']['categories'])) echo ' '.$kaTranslate->translate('Shop:and').' ';
					}
				}
			}
			if(!empty($_GET['filter']['price']['min'])) echo ' '.$kaTranslate->translate('Shop:min price').' <strong>'.floatval($_GET['filter']['price']['min']).'</strong> ';
			if(!empty($_GET['filter']['price']['max'])) echo ' '.$kaTranslate->translate('Shop:max price').' <strong>'.floatval($_GET['filter']['price']['max']).'</strong> ';
			if(!empty($_GET['filter']['status'])) echo ' '.$kaTranslate->translate('Shop:status').' "<strong>'.$kaTranslate->translate('Shop:'. ($_GET['filter']['status']=="draft" ? "Draft" : "On-line")).'</strong>"';

			if(!empty($_GET['filter']['manufacturer']))
			{
				$man=$kaShop->getManufacturer($_GET['filter']['manufacturer']);
				echo ' '.$kaTranslate->translate('Shop:manufacturer').' "<strong>'.$man['name'].'</strong>"';
			}
			?>
			<small><a href="?"><?= $kaTranslate->translate('Shop:remove filters'); ?></a></small>
		</div>
		<br />
		<?php 
	...
	<div class="subset">
		<form method="get" action="">
			<fieldset class="box">
				<legend><?= $kaTranslate->translate('UI:Search'); ?></legend>
				<input type="text" name="search" style="width:180px;" value="<?= htmlentities($_GET['search']); ?>" placeholder="<?= $kaTranslate->translate('Shop:write and press enter...'); ?>">
			</fieldset>
			<br />
			<fieldset class="box closed">
				<h2 onclick="kBoxSwapOpening(this.parentNode);"><?= $kaTranslate->translate('Shop:Filters'); ?></h2>
				
				<?php  if(strpos($pageLayout,",categories,")!==false) { ?>
					<strong><?= $kaTranslate->translate('Shop:Categories'); ?></strong><br>
					...
					function printSubcat($cat)
					{
						...
						?>
						<li>
							<?= b3_create_input("filter[categories][".$cat['data']['idcat']."]","checkbox",$cat['data']['categoria'],$cat['data']['idcat'],"","",(isset($_GET['filter']['categories'][$cat['data']['idcat']])?'checked':''),true); ?></td>
							<?php 
							if(count($cat)>1)
							{
								?><ul><?php 
				...
				<strong><?= $kaTranslate->translate('Shop:Price'); ?></strong><br>
				<?= $kaTranslate->translate('Shop:min'); ?> <input name="filter[price][min]" type="number" value="<?= isset($_GET['filter']['price']['min']) ? $_GET['filter']['price']['min'] : 0; ?>">
				<?= $kaTranslate->translate('Shop:max'); ?> <input name="filter[price][max]" type="number" value="<?= isset($_GET['filter']['price']['max']) ? $_GET['filter']['price']['max'] : 0; ?>"><br>
				<br>
				<strong><?= $kaTranslate->translate('Shop:Status'); ?></strong><br>
				<select name="filter[status]">
					<option value="online" <?= isset($_GET['filter']['status']) && $_GET['filter']['status']=='online' ? 'selected' : ''; ?>><?= $kaTranslate->translate('Shop:On-line'); ?></option>
					<option value="draft"<?= isset($_GET['filter']['status']) && $_GET['filter']['status']=='draft' ? 'selected' : ''; ?>><?= $kaTranslate->translate('Shop:Draft'); ?></option>
					<option value=""<?= !isset($_GET['filter']['status']) || $_GET['filter']['status']=='' ? 'selected' : ''; ?>><?= $kaTranslate->translate('Shop:On-line or draft'); ?></option>
				</select><br>
				<br>
				<?php  if(strpos($pageLayout,",manufacturers,")!==false) { ?>
					<strong><?= $kaTranslate->translate('Shop:Manufacturer'); ?></strong>
						<?php 
						$labels=array("");
						$options=array("");
						...
						$idsman = (!empty($_GET['filter']['manufacturer']) ? $_GET['filter']['manufacturer'] : "");
						echo b3_create_select("filter[manufacturer]","",$labels,$options,$idsman,"100%");
						?>
					</fieldset>
					<br>
				<?php  } ?>
				<input type="submit" value="<?= $kaTranslate->translate('Shop:Apply filters'); ?>" class="smallbutton">
			</fieldset>
		</form>
		<br />
		</div>
		...
		<?php  if(strpos($pageLayout,",privatearea,")!==false&&$kaUsers->canIUse('private')) { ?>
			<div class="box closed"><h2 onclick="kBoxSwapOpening(this.parentNode);"><?= $kaTranslate->translate('Shop:Actions to do if bought'); ?></h2>
				...
				<td>
					...
					if($kaPrivate->getDirContent("") != false)
					{
