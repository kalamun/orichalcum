...
<div id="pager">
	...
	if($tmp[0]['permalink']!="") { ?><a href="<?= $tmp[0]['permalink']; ?>" class="prev" title="<?= str_replace('"','&quot;',$tmp[0]['titolo']); ?>"><?= kTranslate('Older News'); ?></a><? } ?>
	<? $tmp=kGetNewsNext();
	if($tmp[0]['permalink']!="") { ?><a href="<?= $tmp[0]['permalink']; ?>" class="next" title="<?= str_replace('"','&quot;',$tmp[0]['titolo']); ?>"><?= kTranslate('Newer News'); ?></a><? } ?>
	</div>
