...
function kInit($dir=false)
{
	...
	for($i=0;$i<=2;$i++)
	{
		if(!isset($GLOBALS['url'][$i])) $GLOBALS['url'][$i]="";
	}
	...
	$GLOBALS['__texts']=new kText();
}

// backward compatibility... deprecated, to be removed in Orichalcum 3.0
...
/*
return a configuration variable for the current or given language
each variable consists of one param and of two values: you must specify which value do you want
*/
function kGetVar($param, $value=1, $ll=false)
{
	if($ll==false) $ll=LANG;
	$ll = strtoupper($ll);
	...
	{
		return $GLOBALS['__template']->getVar($param, $value, $ll);
	} else {
...
/* return a part of the URI:
eg: www.mywebsite.tld/it/shop/category/item.html
$part=0 => shop
$part=1 => category
$part=2 => item.html
*/
function kGetURIpart($part=0)
{
	if(empty($GLOBALS['url'][$part])) return "";
	else return $GLOBALS['url'][$part];
}
...
function kGetFooter()
{
	...
	$footer = $GLOBALS['__texts']->formatText($footer);
	list($footer,$trash) = $GLOBALS['__texts']->embedImg($footer);
	list($footer,$trash) = $GLOBALS['__texts']->embedDocs($footer);
	return $footer;
}
...
{
	if(empty($GLOBALS['__template']->imgDB['class'])) return "";
...
{
	if($GLOBALS['__dir__']==trim(kGetVar('dir_photogallery',1),"/") && $GLOBALS['__subsubdir__']!="")
	{
...
function kGetPhotogalleryList($vars=false,$start=false,$conditions="",$options="")
{
	if(!is_array($vars))
	{
		$vars=array('limit'=>$vars);
		$vars['offset']=$start;
		$vars['conditions']=$conditions;
		$vars['options']=$options;
		if($vars['limit']==false) $vars['limit']=9999;
		if($vars['offset']==false) $vars['offset']=0;
	}
	return $GLOBALS['__photogallery']->getList($vars);
}

function kPrintPhotogalleryList($vars=false,$start=false,$conditions="",$options="")
{ // DEPRECATED
	if(!is_array($vars))
	{
		$vars=array('limit'=>$vars);
	...
	foreach($GLOBALS['__photogallery']->getList($vars) as $n)
	{
...
function kGetPhotogalleryDateModified()
{
	return $GLOBALS['__pages']->getGalleryVar('modified');
}

function kGetPhotogalleryFeaturedImage()
{
	return $GLOBALS['__pages']->getGalleryVar('featuredimage');
}
...
function kGetPhotogalleryCategories()
{
	$output=$GLOBALS['__photogallery']->getGalleryVar('categories');
	return $output;
}

function kGetPhotogalleriesCategories($vars=array())
{
	$vars['table']=TABLE_PHOTOGALLERY;
	return $GLOBALS['__template']->getCategories($vars);
}


function kGetPhotogalleryCurrentCategoryName()
{
	if(!$GLOBALS['__photogallery']->photogalleryExists()) return false;
	$cat=$GLOBALS['__photogallery']->getCategoryByDir($GLOBALS['__subdir__']);
	if(isset($cat['categoria'])) return $cat['categoria'];
	return false;
}

function kGetPhotogalleryCategoryByDir($dir=null)
{
	if($dir==null&&kIsPhotogallery()) $dir=$GLOBALS['__subdir__'];
	return $GLOBALS['__template']->getCategory(array("table"=>TABLE_PHOTOGALLERY,"dir"=>$dir));
}

function kGetPhotogalleryParentCategories($dir=null)
{
	if($dir==null) $dir=$GLOBALS['__subdir__'];
	return $GLOBALS['__template']->getParentCategories(array("table"=>TABLE_PHOTOGALLERY,"dir"=>$dir));
}

function kGetPhotogalleryCategoryByName($name,$ll=false)
{
	return $GLOBALS['__photogallery']->getCatByName($name,$ll);
}

function kGetPhotogalleryCategoryById($idcat)
{
	return $GLOBALS['__photogallery']->getCatById($idcat);
}
...
function kHaveShopCategory()
{
	return $GLOBALS['__shop']->shopCategoryExists();
}
...
function kGetShopItemPrevious($vars=array())
{
	return $GLOBALS['__shop']->getPrevious($vars);
}

function kGetShopItemNext($vars=array())
{
	return $GLOBALS['__shop']->getNext($vars);
}
