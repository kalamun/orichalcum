...
class kShop {
	...
	public function init()
	{
		...
		$allowedcats = trim($__template->getVar('shop',2),",");
		if($allowedcats != "*")
		{
			foreach(explode(",",trim($allowedcats,",")) as $cat)
			{
		...
		while($row=ksql_fetch_array($results))
		{
			...
			$this->allthecats[$row['idcat']]['featuredimage'] = ($row['featuredimage']>0 ? $this->imgs->getImage($row['featuredimage']) : array());
			
			$row['description'] = $this->kText->formatText($row['description']);
			$tmp=$this->kText->embedImg($row['description']);
			$row['description'] = $tmp[0];
			$tmp=$this->kText->embedDocs($row['description']);
			$row['description'] = $tmp[0];
			$tmp=$this->kText->embedMedia($row['description']);
			$row['description'] = $tmp[0];
			$this->allthecats[$row['idcat']]['description'] = $row['description'];

			if($allowedcats == "*") $this->allowedcats[$row['idcat']]=true;
		...
		$query="SELECT * FROM ".TABLE_SHOP_CUSTOMFIELDS." WHERE `categories` LIKE '%,*,%' ";
	...
	public function getPrevious($vars=array())
	{
		if(!$this->inited) $this->init();
		
		// process input vars
		if(empty($vars['dir'])) $vars['dir'] = kGetURIpart(2);
		
		if(empty($vars['orderby'])) $vars['orderby'] = $GLOBALS['__template']->getVar('shop-order',1);
		$vars['order'] = (strpos($vars['orderby'], "DESC")!==false ? "ASC" : "DESC" );
		...
		$operator = ($vars['order'] == 'ASC' ? '>' : '<');

		if(empty($vars['category'])) $vars['category'] = kGetURIpart(1);
		if(empty($vars['category'])) $vars['category'] = '*';
		if(is_numeric($vars['category'])) $category = array("idcat" => $vars['category']);
		if($vars['category']!="*") $category = $this->getCategoryByDir($vars['category']);
		
		if(empty($vars['limit'])) $vars['limit'] = 1;
		$vars['limit'] = intval($vars['limit']);

		// get current item
		$query="SELECT `".ksql_real_escape_string($vars['orderby'])."` FROM `".TABLE_SHOP_ITEMS."` WHERE `dir`='".ksql_real_escape_string($vars['dir'])."' LIMIT 1";
		$results=ksql_query($query);
		if($row=ksql_fetch_array($results))
		{
			$output=array();
			$query="SELECT * FROM `".TABLE_SHOP_ITEMS."` WHERE `".ksql_real_escape_string($vars['orderby'])."` ".$operator." '".$row[$vars['orderby']]."' AND ll='".LANG."' AND `public` <= NOW() AND `online`='y' ";
			if($category != "*") $query.=" AND `categorie` LIKE '%,".$category['idcat'].",%' ";
			$query.=" ORDER BY `".ksql_real_escape_string($vars['orderby'])."` ".$vars['order']." LIMIT ".intval($vars['limit']);

			$results=ksql_query($query);
			while($row=ksql_fetch_array($results))
			{
				$output[]=$this->row2output($row, array("orderby"=>"`".$vars['orderby']."` ".$vars['order']));
			}
			return $output;
		}
	...
	public function getNext($orderby="",$dir,$limit=1,$cat="*") {
		...
		if(empty($vars['orderby'])) $vars['orderby'] = $GLOBALS['__template']->getVar('shop-order',1);
		$vars['order'] = (strpos($vars['orderby'], "DESC")!==false ? "DESC" : "ASC" );
		$vars['orderby'] = str_replace(" ASC", "", $vars['orderby']);
		$vars['orderby'] = str_replace(" DESC", "", $vars['orderby']);
		$vars['orderby'] = str_replace("`", "", $vars['orderby']);
		$vars['orderby'] = trim($vars['orderby']);

		$operator = ($vars['order'] == 'DESC' ? '<' : '>');
	...
	private function row2output($row,$vars=array()) {
		...
		foreach($this->cats as $cat=>$true)
		{
			if(strpos($row['categorie'],','.$cat.',')!==false)
			{
				...
				if($GLOBALS['url'][0]==$GLOBALS['__template']->getVar('dir_shop',1) && $GLOBALS['url'][1]==$this->allthecats[$cat]['dir']) $subdir = $GLOBALS['__subdir__'];
		...
		if($row['featuredimage']==0) $output['featuredimage']=array();
	...
	*/
	...
	public function getOrderVar($param)
	{
		...
		if(empty($this->orderDB[$param])) return false;
	...
	/*
	Add a payment to an order. The payment could not cover the total amount of the order, but the order will be confirmed only when the total of payments reaches the order amount
	*/
	public function addPayment($uid, $value, $idspay, $details, $currency=false)
	{
		...
		$idspay = intval($idspay);

		if($value>0)
		{
			// set the current order and get basic data
			kSetShopOrderByNumber($uid);
			$o = kGetShopOrderByNumber($uid); // the order
			$idord = $o['idord'];
			$p = $this->getPaymentById($idspay); // the payment method

			// exit if the order not exists
			if(empty($idord)) return false;
			
			// get default currency
			if(empty($currency)) $currency = kGetShopCurrency();
			
			// add payment to the database of payments
			...
			if(!ksql_query($query))
			{
				trigger_error('Error while adding payment of the order '.$uid.' to the database');
				return false;
			}
			
			// if the payment has covered the total amount of the order...
			$totalamount = kGetShopOrderTotalAmount();
			if($totalamount <= $value)
			{
				// ...set order as payed
				$query="UPDATE `".TABLE_SHOP_ORDERS."` SET `payed`='s' WHERE `idord`='".$idord."' LIMIT 1";
				if(!ksql_query($query))
				{
					trigger_error('Error while setting the order '.$uid.' as payed');
					return false;
				}
	...
	function sendEmail($type, $uid)
	{
		...
		// proceed only if the subject of the email is not empty
		if( strip_tags(trim(kGetVar('shop-mail_'.$type, 1))) != "")
		{
			...
			// fill the unset vars
			foreach( array( 'tracking_url', 'tracking_number', 'shipping_address', 'uid', 'invoice_data', 'totalprice', 'idspay', 'iddel', 'personal_data', 'invoice_data', 'shipping_address') as $v )
			{
				if(empty($o[$v])) $o[$v]='';
			}
			
			// fill the unset payment method vars
			foreach( array('name', 'descr', 'zones', 'price', 'price_percent', 'gateway') as $v )
			{
				if(empty($o['payment_method'][$v])) $o['payment_method'][$v]='';
			}
			
			// fill the unset carrier method vars
			foreach( array('name', 'zones', 'prices') as $v )
			{
				if(empty($o['deliverer'][$v])) $o['deliverer'][$v]='';
			}
			
			// fill the unset member vars
			foreach( array('name', 'email', 'username', 'password') as $v )
			{
				if(!isset($o['member'][$v])) $o['member'][$v]='';
			}
			...
			// get the e-mail text and replace the placeholders
			$mail['subject'] = kGetVar('shop-mail_'.$type, 2);
			$mail['message'] = kGetVar('shop-mail_'.$type, 1);
			
			// if the mail body is empty, return
			if(trim(strip_tags($mail['message'],"<img>")) == "") return true;
			...
			{
				$invoice_data = nl2br(strip_tags($o['invoice_data']));
			}

			foreach($mail as $k=>$v)
			{
				$mail[$k]=str_replace("{NAME}", $o['member']['name'], $mail[$k]);
				$mail[$k]=str_replace("{EMAIL}", $o['member']['email'], $mail[$k]);
				$mail[$k]=str_replace("{USERNAME}", $o['member']['username'], $mail[$k]);
				$mail[$k]=str_replace("{PASSWORD}", $o['member']['password'], $mail[$k]);
				$mail[$k]=str_replace("{ADDRESS}", $address, $mail[$k]);
				$mail[$k]=str_replace("{PERSONAL_DATA}", $personal_data, $mail[$k]);
				$mail[$k]=str_replace("{DELIVERER}", $o['deliverer']['name'], $mail[$k]);
				$mail[$k]=str_replace("{CARRIER}", $o['deliverer']['name'], $mail[$k]);
				$mail[$k]=str_replace("{TRACKING_URL}", $o['tracking_url'], $mail[$k]);
				$mail[$k]=str_replace("{TRACKING_NUMBER}", $o['tracking_number'], $mail[$k]);
				$mail[$k]=str_replace("{SHIPPING_ADDRESS}", $shipping_address, $mail[$k]);
				$mail[$k]=str_replace("{PAYMENT_METHOD}", '<strong>'.$o['payment_method']['name'].'</strong> '.$o['payment_method']['mail_instructions'], $mail[$k]);
				$mail[$k]=str_replace("{ORDER_ITEMS}", $items, $mail[$k]);
				$mail[$k]=str_replace("{ORDER_NUMBER}", $o['uid'], $mail[$k]);
				$mail[$k]=str_replace("{BILLING_DATA}", $invoice_data, $mail[$k]);
				$mail[$k]=str_replace("{INVOICE_DATA}", $invoice_data, $mail[$k]);
				$mail[$k]=str_replace("{ORDER_PRICE}", number_format($o['totalprice'], 2).' '.kGetShopCurrency("symbol"), $mail[$k]);
				$mail[$k]=str_replace("{PAYMENT_PRICE}", number_format($this->getCartPaymentPrice($o['totalprice'], $o['idspay'], $o['iddel'],$countries[0]['ll']), 2).' '.kGetShopCurrency("symbol"), $mail[$k]);
				$mail[$k]=str_replace("{SHIPPING_PRICE}", number_format($this->getCartShippingPrice($o['iddel'], $countries[0]['ll']), 2).' '.kGetShopCurrency("symbol"), $mail[$k]);
			}
