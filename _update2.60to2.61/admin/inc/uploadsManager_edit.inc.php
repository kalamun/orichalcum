...
{ ?>
	...
	<div class="padding">
		...
		/* EDIT FILES */
			...
			<form action="" method="post" enctype="multipart/form-data">
				...
				<tr><td align="center"><h2>Immagine</h2><img src="<?= ($img['hotlink']==false?BASEDIR:'').$img['url']; ?>" height="100" alt="" /></td>
					<td style="vertical-align:middle;"><label ref="img"><?= $kaTranslate->translate('UI:Change'); ?></label><br /><?
						if($img['hotlink']==false) { ?><input name="img" type="file" id="img" /> <input name="saveimg" type="submit" value="<?= $kaTranslate->translate('Img:Upload picture'); ?>" class="smallbutton" /><? }
						else { ?><input type="text" name="img" value="<?= str_replace('"','&quot;',$img['url']); ?>" style="width:300px;"> <input name="save" type="submit" value="<?= $kaTranslate->translate('Img:Update hotlink'); ?>" class="smallbutton" /> <input name="hotlinktoimg" type="submit" value="<?= $kaTranslate->translate('Img:Import to your website'); ?>" class="smallbutton" /><? }
						?><br />
		...
		elseif($_GET['action']=="istances") {
			?><h2><?= $kaTranslate->translate('Img:Usage'); ?></h2><br /><?
