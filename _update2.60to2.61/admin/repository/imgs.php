...
<div class="topset">
	<div class="box">
		...
		<?
		if($_GET['p']>1) { ?><a href="?p=<?= $_GET['p']-1; ?>&q=<?= urlencode($_GET['q']); ?>">&lt; <?= $kaTranslate->translate('Repository:previous'); ?></a><? }

		$tot=$kaImages->countList(false,false,$conditions);
		for($i=1;$i<=ceil($tot/$items4page);$i++)
		{
			?><a href="?p=<?= $i; ?>&q=<?= urlencode($_GET['q']); ?>" class="<?= ($i==$_GET['p'])?'selected':''; ?>"><?= $i; ?></a><?
		}

		if($_GET['p']<ceil($tot/$items4page)) { ?><a href="?p=<?= $_GET['p']+1; ?>&q=<?= urlencode($_GET['q']); ?>"><?= $kaTranslate->translate('Repository:next'); ?> &gt;</a><? }
		?>
	</div>
