...
<header>
	<h1><span class="icon"></span> <?= $kaImpostazioni->getVar("sitename",1); ?></h1>&nbsp;&nbsp;<?= $kaTranslate->translate('Menu:Control Panel'); ?>
	<div class="logout">
		...
		<?php  if(strpos($_SESSION['permissions'],',upgrade,')!==false) { ?><a href="<?= ADMINDIR; ?>upgrade/" title="Aggiorna Bettino"><?= $kaTranslate->translate('Menu:Upgrade'); ?></a> | <?php  } ?>
		<a href="?logout"><?= $kaTranslate->translate('Menu:Logout'); ?> <span class="icon"></span></a>
	</div>
</header>
...
<div id="contents">
	<?php  if($kaAdminMenu->getSelected('id')!="") { ?>
	<div id="submenu">
		<div class="submenu">
		<ul><li>
			<h2><span class="icon"><?= $kaAdminMenu->getSelected('icon'); ?></span> <?= $kaAdminMenu->getSelected('title'); ?></h2>
			<?php 
			/* print left menu */
