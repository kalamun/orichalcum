...
/* MYSQL to PDO */
function ksql_connect($host,$dbname,$user,$password,$port="")
{
	try
	{
		$GLOBALS['__db']['pdo']=new PDO('mysql:host='.$host.';'. (!empty($port) ? 'port='.$port.';' : '' ) .'dbname='.$dbname , $user, $password);
...
/* CONNECT TO DB */
if(!isset($GLOBALS['__db']['port'])) $GLOBALS['__db']['port']="";
$GLOBALS['__db']['id'] = ksql_connect($GLOBALS['__db']['host'], $GLOBALS['__db']['name'], $GLOBALS['__db']['user'], $GLOBALS['__db']['password'], $GLOBALS['__db']['port']);
