<?php 
define("PAGE_NAME","Pages:Categories management");
...
if(isset($_POST['save'])&&count($_POST['cat'])>0) {
	...
	foreach($order as $ref=>$categories) {
		foreach($categories as $ordine=>$idcat) {
			if(!$kaCategorie->updateOrder($idcat,$ordine+1,$ref,TABLE_PAGINE)) $log="Errore durante il salvataggio dell'ordinamento";
			}
		}
...
elseif(isset($_POST['sortAZ'])) {
	$log=$kaCategorie->sortby('`categoria` ASC',TABLE_PAGINE);
	}
...
if(isset($_GET['idcat'])) {
	...
	<form action="" method="post" id="orderby" onsubmit="saving();">
		<div class="dragdrop">
			<ul class="DragZone"><?php 
				...
				$categorie=$kaCategorie->getStructuredList(TABLE_PAGINE);
		...
		<div class="submit" id="submit">
			<input type="submit" name="save" class="button" value="<?= $kaTranslate->translate('Pages:Save order'); ?>" />
			<input type="submit" name="sortAZ" class="smallbutton" value="<?= $kaTranslate->translate('Pages:Sort A-Z'); ?>" />
			</div>
	</form>
	...
	<td><div id="nuovaCat" style="display:none;">
		<fieldset class="box"><legend><?= $kaTranslate->translate('Pages:Add a new category'); ?></legend>
		<form action="" method="post">
		...
		<div class="URLBox"><?= b3_create_input("dir","text",$kaTranslate->translate('Pages:Category URL').": ".BASEDIR.strtolower($_SESSION['ll'])."/".$kaImpostazioni->getVar('dir_shop',1).'/',(isset($copyfrom['dir'])?$copyfrom['dir'].'-'.date("Ymd"):''),"400px",64,'onkeyup="checkURL(this)"'); ?> <span id="dirYetExists" style="display:none;">Questo indirizzo esiste gi&agrave;!</span></div><br />
