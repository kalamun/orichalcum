<?php 
define("PAGE_NAME","News:Categories management");
...
if(isset($_POST['save'])&&count($_POST['cat'])>0) {
	...
	foreach($order as $ref=>$categories) {
		foreach($categories as $ordine=>$idcat) {
			if(!$kaCategorie->updateOrder($idcat,$ordine+1,$ref,TABLE_NEWS)) $log="Errore durante il salvataggio dell'ordinamento";
			}
		}
...
elseif(isset($_POST['sortAZ'])) {
	$log=$kaCategorie->sortby('`categoria` ASC',TABLE_NEWS);
	}
...
if(isset($_GET['idcat'])) {
	...
	<form action="" method="post" id="orderby" onsubmit="saving();">
		<div class="dragdrop">
			<ul class="DragZone"><?php 
				...
				$categorie=$kaCategorie->getStructuredList(TABLE_NEWS);
		...
		<div class="submit" id="submit">
			<input type="submit" name="save" class="button" value="<?= $kaTranslate->translate('News:Save order'); ?>" />
			<input type="submit" name="sortAZ" class="smallbutton" value="<?= $kaTranslate->translate('News:Sort A-Z'); ?>" />
			</div>
	</form>
	...
	<script type="text/javascript" src="js/categorie.js"></script>
	<table><tr>
	<td><input type="button" class="button" value="<?= $kaTranslate->translate('News:New category'); ?>" onclick="showReq('nuovaCat');" /></td>
	<td><div id="nuovaCat" style="display:none;">
		<fieldset class="box"><legend><?= $kaTranslate->translate('News:Add a new category'); ?></legend>
		<form action="" method="post">
		<div class="title"><?= b3_create_input("titolo","text",$kaTranslate->translate('News:Category name')."<br />","","95%",250,'autocomplete="off" onkeyup="title2url()" onblur="titleBlur()"'); ?></div>
		<div class="URLBox"><?= b3_create_input("dir","text",$kaTranslate->translate('News:Category URL').": ".BASEDIR.strtolower($_SESSION['ll'])."/".$kaImpostazioni->getVar('dir_shop',1).'/',(isset($copyfrom['dir'])?$copyfrom['dir'].'-'.date("Ymd"):''),"400px",64,'onkeyup="checkURL(this)"'); ?> <span id="dirYetExists" style="display:none;">Questo indirizzo esiste gi&agrave;!</span></div><br />
